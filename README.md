...
| Results §4.2 ("SAE reconstruction fails to preserve membership signal recoverable from the residual"); Table `tab:dark_subspace` | Pythia-6.9B N=5 mixed-data SAE reconstruction reduction | `scripts/dark_subspace/p69_n5_harmonize.py` | `results/dark_subspace/paper_claims/p69_n5_harmonized_2026-05-06.json`. Six SAEs were trained on Pythia-6.9B at identical hyperparameters (layer 16, multiplier 4, L1 5e-4, 200M tokens, dead-feature resampling); the only difference between runs is the random initialisation of the SAE weights. `tab:dark_subspace` reports the cohort at N=5 to give every multi-seed Pythia row in the table the same five-seed reporting basis (the Pythia-1B row in the same table also reports five seeds, $\{42, 43, 44, 45, 46\}$). The N=5 cohort follows the same seed list, with the local naming variant `42_postfix` recorded canonically as seed 42 (see the JSON `seed_label` field). The single non-overlap with the trained set is seed 47, the chronologically last of the six runs (training timestamp 2026-04-16 15:52); it is omitted from the N=5 reporting set rather than dropped from the artefact. The shipped JSON contains both the N=5 cluster summary and the full N=6 reference cluster summary, the per-seed metrics for all six runs (including seed 47), and the per-metric N=5 minus N=6 deltas. The JSON's `materiality.verdict` field reports `NEGLIGIBLE`, with $\lvert\Delta_{\mathrm{mean}}(N{=}5 - N{=}6)\rvert < 0.005$ on every reported metric (`original_score_K_auroc`, `reconstructed_score_K_auroc`, `residual_score_K_auroc`, `drop_original_minus_reconstructed`, `recon_cos`). The `verify_claims.py` script asserts that this materiality verdict is `NEGLIGIBLE`; the conclusion does not depend on the choice of N=5 versus N=6. |
...
| Methods §3.5 / Results §4.4 (`tab:dd_full`, `tab:dd_extraction`, `tab:epoch_dd`, `app:per_model_dd`): subspace erasure with downstream membership detection and verbatim extraction | Membership AUROC, mean member loss, exact-match rate, and extraction ROUGE-L under $S_K$/$S_R$ erasure (Methods Eq. 3) | `scripts/dark_subspace/dd_table_render.py` renders the table from per-cell records produced by the GPU pipeline (model load, basis fit, forward-pass erasure hook, decoded continuations, ROUGE-L scoring); see the script docstring for the per-cell record schema. | `results/dark_subspace/paper_claims/double_dissociation.json` (bundled). Encodes the manuscript-rendered cell values for `tab:dd_full` (4 model rows × 2 metrics × 4 conditions), `tab:dd_extraction` (8 model rows × 4 conditions, ROUGE-L), and `tab:epoch_dd` (3 epochs × 2 metrics × 4 conditions); the verifier asserts cross-table consistency (Pythia-1B at epoch 5 must match across `tab:dd_extraction` and `tab:epoch_dd`) and the row-count and erase-condition $\leq$ baseline invariants. The per-cell GPU-pipeline records that produce these values regenerate to `runs/memcirc/double_dissociation/<model_tag>/double_dissociation.json` and are not bundled because each carries SAE checkpoint references and decoded-text arrays. |
