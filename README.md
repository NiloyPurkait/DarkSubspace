...
| Appendix: "L2-normalised residual membership AUROC" (`tab:l2_normalized`) | Norm-versus-direction split of residual signal | `scripts/dark_subspace/l2_normalized_auroc.py` plus `scripts/dark_subspace/norm_baseline.py` | The activation-norm half of the table is reproducible from the shipped `norm_baseline/*/results.json`. The L2-normalised residual AUROC half writes to `results/dark_subspace/generated/l2_normalized/<model_tag>/results.json` when the script is rerun against the matching SAE-residual scores. |
