...
| Appendix: "Pythia-12B Replication Detail"; Table `tab:dark_subspace` | Pythia-12B mixed-SAE three-init replication (seeds 47-49 shipped here; see Provenance Notes) | `scripts/dark_subspace/p12b_multiseed_query.py`, `scripts/dark_subspace/shell/sbatch_p12b_multiseed_array.sh` | `results/dark_subspace/generated/sae_dark_subspace/p12b_mixed_sae_seed{47,48,49}/results.json` |
...
A few naming conventions in shipped paths warrant brief explanation.

- **`_v2` suffix in `behavioral_channels/{falcon7b,llama3,mistral}_epoch5_v2/` directories.** The `_v2` suffix marks the canonical re-run used by `scripts/dark_subspace/figure_data_loader.py`. The loader prefers `_v2` when both the original and re-run directories exist (see the comment block in `figure_data_loader.py`'s `MODEL_REGISTRY`). The `_v2` runs use the same `behavioral_channels.py` driver but with corrected per-model fine-tuned model paths recorded in `scripts/dark_subspace/shell/multiseed/sbatch_falcon.sh` and the corresponding sister scripts.
- **`p69_mixed_sae_v2/`** is referenced in `scripts/dark_subspace/figure_data_loader.py` as the canonical Pythia-6.9B mixed-data SAE evaluation directory. The full directory is not bundled in `results/dark_subspace/generated/sae_dark_subspace/`. The N=5 mixed-data cohort numbers used in the paper are aggregated and shipped via `results/dark_subspace/paper_claims/p69_n5_harmonized_2026-05-06.json`.
- **`seed42_postfix` (Pythia-6.9B).** The `scripts/dark_subspace/shell/sbatch_p69_seed42_postfix.sh` job retrains the original seed-42 Pythia-6.9B mixed-data SAE under the post-corpus-cycling-fix training pipeline, to disambiguate whether the L0=551 vs L0=11 gap between the original seed-42 SAE and the seed-43 SAE was seed variance or a pre-fix training artefact. The script header documents the diagnostic in detail.
- **Pythia-12B SAE seeds.** The paper claims five SAE seeds (47-51) per Appendix `app:p12b_replication`. The shipped JSONs cover seeds 47, 48, and 49 (`results/dark_subspace/generated/sae_dark_subspace/p12b_mixed_sae_seed{47,48,49}/results.json`). The driver `scripts/dark_subspace/shell/sbatch_p12b_multiseed_array.sh` is configured for seeds 47-51 (`#SBATCH --array=0-4`). Seeds 50 and 51 are not shipped in this artefact; the verifier's Pythia-12B replication checks operate on the three shipped seeds.
