...
```text
Asserted check summary: 161/161 pass, 0 fail
...
| Results §4.2 ("SAE reconstruction fails to preserve membership signal recoverable from the residual"); Table `tab:dark_subspace` | Pythia-6.9B N=5 mixed-data SAE reconstruction reduction | `scripts/dark_subspace/p69_n5_harmonize.py` | `results/dark_subspace/paper_claims/p69_n5_harmonized_2026-05-06.json`. The Pythia-6.9B mixed cohort was trained at six seeds ({42_postfix, 43, 44, 45, 46, 47}). The N=5 row reported in `tab:dark_subspace` keeps {42_postfix, 43, 44, 45, 46} and drops seed 47, so that the Pythia-6.9B and Pythia-1B rows share an identical seed list ({42, 43, 44, 45, 46}). The drop is recorded in the JSON's `harmonisation_pattern` and `seeds_dropped` fields, and the underlying N=6 cluster summary is preserved alongside the N=5 cluster in the same file. |
