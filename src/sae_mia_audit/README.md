...
| `utils/` | HuggingFace spec parsing (`HFModelSpec`), structured logging (`setup_logging`, `get_logger`), global seed configuration (`SeedConfig`, `set_global_seed`), and timestamped output directories with reproducibility-snapshot helpers (`make_run_dir`, `snapshot_reproducibility`, `dataclass_to_dict`). | All paper scripts. The `run_dir` helpers are used by `scripts/shared/train_sae.py`. |
