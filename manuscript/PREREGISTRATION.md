...
- **Layer.** Per-model layer chosen before the dark-subspace experiment. Pythia 6.9B: layer 16. Pythia 12B: layer 18. GPT-Neo 2.7B: layer 16. Qwen2 7B: layer 16. Other models: see `scripts/dark_subspace/configs/oc_roster.json`.
- **Reproducibility.** All probe and BCD scripts under `scripts/dark_subspace/` use deterministic seeds (default 42).
...
- Source: `runs/dark_subspace/bow_ceiling/memcirc_ctrl_ft/results.json`.
...
- Source: `runs/dark_subspace/paraphrase_sensitivity/{p69,qwen2,p12b}/results.json`.
...
- **Pre-FT baseline.** BCD probe on un-fine-tuned `EleutherAI/pythia-6.9b` at layers 12, 14, 16, 18, 20. Output: `runs/dark_subspace/behavioral_channels/p69_BASE_pre_ft/`. Rules out "is this a base-model property?"
- **Layer sensitivity sweep.** BCD probe at layers 12, 14, 16, 18, 20 on the FT P69 model. Output: `runs/dark_subspace/behavioral_channels/p69_epoch5_layer_sweep/`. Rules out "is layer 16 cherry-picked?"
- **K=5 PC ablation.** Same script as K=10/50/200, just with K=5. Output: `runs/dark_subspace/causal_ablation_K5/`. Tightens the K-sweep claim from below.
- **Random-direction baseline.** Sample 100 random unit directions per model, compute member-vs-nonmember AUROC along each, compare distribution to the residual probe AUROC. Output: `runs/dark_subspace/random_direction_baseline/`. Rules out "any direction in activation space works."
- **Random-init SAE baseline.** Run dark-subspace evaluation against an untrained random-init SAE of matching shape on Pythia 6.9B. Output: `runs/dark_subspace/sae_dark_subspace/p69_random_init_baseline/`. Rules out "any dictionary-shaped operator works."
- **Pythia-12B multi-seed pilot.** First mixed-data SAE seed on Pythia 12B at P69-matched HPs (mult=4, L1=5e-4, layer 18). Output: `runs/dark_subspace/sae_dark_subspace/p12b_mixed_sae_seed42/`. Gate at recon_cos $\geq 0.95$ before scheduling additional seeds for a second multi-seed anchor.
- **GPT-Neo mult=8 pilot.** Mixed-data SAE on GPT-Neo 2.7B at mult=8, L1=5e-4 (versus the failed mult=4 baseline). Output: `runs/dark_subspace/sae_dark_subspace/neo_mixed_mult8_seed42/`. Gate at recon_cos $\geq 0.95$.
...
- **Reason.** Reporting harmonization to align all multi-seed Pythia rows in `tab:dark_subspace` to a uniform N=5 SAE-init reporting convention, matching the Pythia-1B multi-seed cohort seed list. Pattern A (drop seed 47) was selected because seeds 42-postfix through 46 align one-to-one with the Pythia-1B N=5 seed list. Disk evidence is unchanged. The N=6 cohort remains preserved on disk under `runs/dark_subspace/sae_dark_subspace/p69_mixed_sae_seed{42_postfix,43,44,45,46,47}/results.json` and the cached aggregator at `runs/dark_subspace/sae_noise_floor/p69_aggregate.json`. Per-metric materiality from the harmonization memo is NEGLIGIBLE. All five canonical metrics shift by less than 0.005 between N=5 and N=6 cluster means. The 3dp-rounded table cells are identical for original / reconstructed / drop / recon_cos, only residual shifts by 0.002 at 3dp. Originally reported as N=6 mean drop 0.20932 ± 0.00337 at 62 sigma, harmonized 2026-05-06 to N=5 mean drop 0.20938 ± 0.00376 at 56 sigma per `results/dark_subspace/paper_claims/p69_n5_harmonized_2026-05-06.json`. The within-Pythia strong-drop anchor claim (drop above 0.10 at recon_cos above 0.99) survives both reporting conventions unchanged.
- **Cross-reference.**Source JSON `results/dark_subspace/paper_claims/p69_n5_harmonized_2026-05-06.json`. Subselection script `scripts/dark_subspace/p69_n5_harmonize.py`.
...
- **Cross-reference.** Methods sentence at `manuscript/methods.tex` lines 67--68 (pre-registered partition-fit probe). Results paragraph at `results/dark_subspace/paper_claims.tex:125` (Held-out partition-fit probe for $d_K$). Self-contained appendix entry at `manuscript/appendix.tex:955` (`app:heldout_dk`). Limitation in `manuscript/main.tex:127` Discussion. Per-split data at `results/dark_subspace/paper_claims/heldout_dk_2026-05-02.json`. Script at `scripts/dark_subspace/heldout_dk_eval.py`.
...
- **Cross-reference.** SAE validity gate paragraph in `manuscript/methods.tex:59`.

## Revision History

| Date | Change |
|------|--------|
| 2026-05-06 | Added Amendment 2026-05-06 for the Pythia 6.9B multi-seed cohort harmonization from N=6 (seeds 42-postfix, 43, 44, 45, 46, 47) to N=5 (drop seed 47). PR-2 Multi-seed cohort and PR-8 Statistical Reporting bullets updated with strikethrough plus Amendment annotation. Per-metric materiality is NEGLIGIBLE. Source JSON `results/dark_subspace/paper_claims/p69_n5_harmonized_2026-05-06.json` (N=5 mean drop 0.20938 ± 0.00376 at 56 sigma, was 0.20932 ± 0.00337 at 62 sigma under N=6). Disk N=6 cohort preserved unchanged. |
