...
The main erasure experiments remove the leading $n_c = 5$ directions of the relevant subspace. We also run higher-rank ($n_c \in \{10, 20\}$) sensitivity checks on Falcon-7B, Mistral-7B, OPT-6.7B, and Qwen2-7B (the four non-Pythia architectures where rank-5 erasure leaves residual extraction above 0.10), and lower-rank ($n_c \in \{1, 2\}$) sensitivity checks on Pythia-6.9B (epoch 3) and Llama-3-8B (epoch 3) to bracket the rank threshold for verbatim collapse on the Pythia and Llama-family models. Per-model metric tables for both sensitivity directions are in Appendix~\ref{app:dd_full}. The verbatim-extraction measurement uses ROUGE-L on greedy continuations from fixed-length prefixes, with prefixes truncated to a common length across all models so that prefix exposure is held constant when comparing extraction outcomes across the erasure conditions.
...
The run record is \texttt{manuscript/\allowbreak{}results/\allowbreak{}heldout\_\allowbreak{}dk\_\allowbreak{}2026-\allowbreak{}05-\allowbreak{}02.json}.
...
The run record is \texttt{manuscript/\allowbreak{}results/\allowbreak{}k\_\allowbreak{}oc2\_\allowbreak{}bootstrap\_\allowbreak{}residual\_\allowbreak{}minus\_\allowbreak{}reconstruction\_\allowbreak{}2026-\allowbreak{}05-\allowbreak{}05.json}.
...
The pooled AUROC is 0.4566, rounded to 0.457, so this token-level baseline does not distinguish members from non-members on the controlled split. The result argues against a simple unigram vocabulary explanation of the residual signal. It does not rule out higher-order surface artefacts, since the bag-of-words representation discards word order and context. The numerical record is \texttt{runs/\allowbreak{}memcirc/\allowbreak{}bow\_\allowbreak{}ceiling/\allowbreak{}memcirc\_\allowbreak{}ctrl\_\allowbreak{}ft/\allowbreak{}results.json}.

\subsection{Length-feature baseline}
\label{app:length_baseline}

We compute three length-derived membership classifiers on the controlled OWT cohort. Character length, word count, and vocabulary richness (unique-token fraction). On $n_{\mathrm{member}}=n_{\mathrm{nonmember}}=1000$, the AUROCs are 0.502 (character length), 0.509 (word count), and 0.507 (vocabulary richness), all at chance. Mean character lengths are 889.6 (member) and 889.1 (non-member), confirming that the controlled cohort is length-matched by construction. Surface text length cannot account for the membership separability we observe at the activation level.

\subsection{Label-shuffled cosine null}
\label{app:label_shuffled_null}

To check that the cosine alignment between the knowledge direction $\dK$ and the recall direction $\dR$ is not an artifact of label assignment, we shuffle the (member, non-member) and (high-recall, low-recall) labels independently within the controlled cohort and recompute the corresponding directions. Across $n=1000$ random label permutations per model, the observed $|\cos(\dK, \dR)|$ is consistent with the null distribution on 8 of the 9 models in the cohort (per-model $p$-values range from 0.162 to 1.000, no significant deviation at $\alpha=0.05$). On Mistral-7B the observed cosine is significantly lower than chance ($p=0.037$), supporting rather than violating the near-orthogonality claim. The cosine alignment is therefore not driven by label structure on any model.
...
The controls paragraph in \S\ref{sec:results:controls} groups several checks by the alternative explanations they address. This entry gives the per-control method and outcome. Per-control numerical detail is recorded under \texttt{runs/dark_subspace/}, with seeds and configs documented in each run's \texttt{config.json}.
...
Table~\ref{tab:tpr_at_0p1pct_fpr} reports bootstrap-resampled TPR at the 0.1\% FPR cutoff for the residual $\dK$ membership channel on the four-model baseline cohort. The 0.1\% FPR cutoff is the operating point used by recent fine-tuning MIA work~\citep{chen2026wbc, ilic2026ezmia}. We use $n_{\mathrm{boot}}=10{,}000$ bootstrap resamples drawn with replacement from the per-text-score level, with seed 12345 and $n_{\mathrm{member}}=n_{\mathrm{nonmember}}=1000$ per model.

\begin{table}[!htbp]
\centering
\caption{TPR at 0.1\% FPR for residual $\dK$ across four models. Point estimates and bootstrap 95\% CIs from $n_{\mathrm{boot}}=10{,}000$ resamples. Pythia-12B has the highest point estimate at low FPR, with overlapping bootstrap CIs across the three non-Qwen2 models. Qwen2-7B is not statistically distinguishable from chance at this sample size, consistent with Qwen2's weak architectural replication of the dark-subspace effect documented in Appendix~\ref{app:qwen2_pilot}.}
\label{tab:tpr_at_0p1pct_fpr}
\appCompactTable{\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Model} & \textbf{TPR (point)} & \textbf{Bootstrap mean} & \textbf{95\% CI} \\
\midrule
Pythia-6.9B   & 0.015 & 0.018 & [0.005, 0.050] \\
Pythia-12B    & 0.032 & 0.035 & [0.020, 0.071] \\
GPT-Neo-2.7B  & 0.025 & 0.025 & [0.014, 0.042] \\
Qwen2-7B      & 0.010 & 0.007 & [0.000, 0.023] \\
\bottomrule
\end{tabular}}
\end{table}

Three of the four models (Pythia-6.9B, Pythia-12B, GPT-Neo-2.7B) have CI lower bounds excluding the chance baseline of 0.001 at FPR = 0.001. Qwen2-7B's CI lower bound at 0.000 means its TPR at 0.1\% FPR is not statistically distinguishable from chance at this sample size. The bootstrap captures sampling variance over the 1000 plus 1000 evaluation tuples but not SAE training seed variance.
