...
Reproduce::
    ...
    env/bin/python3 scripts/memcirc/aggregate_multiseed.py --model all
"""
...
SAE_BASE = REPO / "runs/memcirc/sae_dark_subspace"
...
def aggregate_one(model_key: str, spec: dict) -> dict:
    ...
    n_landed = len(found_seeds)

    cluster = {
        "n_landed": n_landed,
        "expected_n": spec["expected_n"],
        "complete": n_landed == spec["expected_n"],
...
def print_human(model_key: str, agg: dict):
    ...
    if n == 0:
        print("  NO results, all seeds pending or missing.")
        return
    for metric in ["orig_auroc", "recon_auroc", "residual_auroc", "drop", "recon_cos"]:
        m = c[f"{metric}_mean"]
        s = c[f"{metric}_stdev"]
...
def main():
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "paper5/results/multiseed_<model>.json per model and a combined "
            "paper5/results/multiseed_cluster.json."
        ),
    )
    args = ap.parse_args()

    out_dir = REPO / "paper5/results"
