...
CPU-only paper-numerical-claim verifier for the Dark Subspace paper. Reads each
JSON cited by the paper, asserts the on-disk values match the paper-text values
to within a small numerical tolerance, and exits 1 on any mismatch.
...
for label, dirname, layer in bcd_models_main:
    p = ROOT / f"runs/memcirc/behavioral_channels/{dirname}/orthogonality.json"
    d = load(p)
    if d is None:
        continue
...
p = ROOT / "runs/memcirc/behavioral_channels/p69_BASE_pre_ft/orthogonality.json"
d = load(p)
if d is not None:
    layers = d.get("per_layer", {})
...
p = ROOT / "runs/memcirc/behavioral_channels/p69_epoch5_layer_sweep/orthogonality.json"
d = load(p)
if d is not None:
    layers = d.get("per_layer", {})
    print(f"  Available layers: {list(layers.keys())}")
...
table2_bottom = [
    ...
    ("Mistral-7B (excluded but old row)", "mistral_epoch5"),
]
for label, dirname in table2_bottom:
    p = ROOT / f"runs/memcirc/sae_dark_subspace/{dirname}/results.json"
    d = load(p)
    if d is None:
        continue
...
# Pythia-6.9B mixed-data SAE row reads the CANONICAL N=6 aggregate, not a single run.
# The single-run dir runs/memcirc/sae_dark_subspace/p69_mixed_sae/ is a stale superseded snapshot.
print("\nP69 mixed-data SAE (canonical N=6 aggregate):")
agg = load(ROOT / "runs/memcirc/sae_noise_floor/p69_aggregate.json")
if agg is not None:
    s = agg.get("summary", {})
    o_n6 = s.get("original_score_K_auroc", {}).get("mean")
    ...
    print(f"  N={s.get('original_score_K_auroc', {}).get('n')} aggregate: orig={o_n6:.4f}  recon={r_n6:.4f}  drop={drop_n6:.4f} (std {drop_std:.4f})  resid={rs_n6:.4f}  rc={rc_n6:.4f}")
    _check("P69 N=6 mixed orig (paper 0.803)", 0.803, o_n6)
    _check("P69 N=6 mixed recon (paper 0.594)", 0.594, r_n6)
    _check("P69 N=6 mixed resid (paper 0.779)", 0.779, rs_n6)
    _check("P69 N=6 mixed recon_cos (paper 0.976)", 0.976, rc_n6)
    _check("P69 N=6 mixed drop_mean (paper 0.209)", 0.209, drop_n6)
...
for seed, expected_drop in p12b_seeds:
    p = ROOT / f"runs/memcirc/sae_dark_subspace/p12b_mixed_sae_seed{seed}/results.json"
    d = load(p)
    if d is None:
...
import os
nb_dir = ROOT / "runs/memcirc/norm_baseline"
if nb_dir.exists():
...
for tag, layer_pref in [("p70m_epoch5", None), ("p160m_epoch5", None), ("p410m_epoch5", None),
                        ...
                        ("p12b_epoch5", 24)]:
    p = ROOT / f"runs/memcirc/behavioral_channels/{tag}/orthogonality.json"
    d = load(p)
    if d is None:
        continue
    layers = d.get("per_layer", {})
...
for ep in ["p1b_epoch1", "p1b_epoch3", "p1b_epoch5"]:
    p = ROOT / f"runs/memcirc/behavioral_channels/{ep}/orthogonality.json"
    d = load(p)
    if d is None:
        continue
    layers = d.get("per_layer", {})
...
# -----------------------------------------------------------------------------
# Standard MIA probes (paper status: pending check)
# -----------------------------------------------------------------------------
banner("Standard MIA probes")
p = ROOT / "runs/memcirc/standard_mia_probes/p69_dark_subspace_replication/results.json"
d = load(p)
if d is None:
    print("  Standard MIA probes still PENDING. Paper has no claim depending on this yet.")
else:
...
bib = ROOT / "paper5/references.bib"
if bib.exists():
    text = bib.read_text()
    found = {
        "meeus2025sok": "meeus2025sok" in text,
        "duan2024membership": "duan2024membership" in text,
        "muhamed2025dsg": "muhamed2025dsg" in text,
        "engels2024dark": "engels2024dark" in text,
        "wang2025sspu": "wang2025sspu" in text,
        "frikha2025privacyscalpel": "frikha2025privacyscalpel" in text,
        "suri2025mitigating": "suri2025mitigating" in text,
        "carlini2021extracting": "carlini2021extracting" in text,
        "deng2026projection": "deng2026projection" in text,
        "paudel2026sanitization": "paudel2026sanitization" in text,
        "huang2026gdrift": "huang2026gdrift" in text,
        "minder2025narrow": "minder2025narrow" in text,
        "galli2024lumia": "galli2024lumia" in text,
        "ferrando2026memtrace": "ferrando2026memtrace" in text,
        "marks2024sparse": "marks2024sparse" in text,
        "leask2025sparse": "leask2025sparse" in text,
        "peng2025discover": "peng2025discover" in text,
        "grande2026privacyscalpelv3": "grande2026privacyscalpelv3" in text,
        "anonymized2026popets": "anonymized2026popets" in text,
        "hu2025jogging": "hu2025jogging" in text,
        "chen2025unlearning": "chen2025unlearning" in text,
        "zhang2025minkpp": "zhang2025minkpp" in text,
        "yang2024qwen2": "yang2024qwen2" in text,
        "huang2024demystifying": "huang2024demystifying" in text,
        "maini2024dataset": "maini2024dataset" in text,
        "templeton2024scaling": "templeton2024scaling" in text,
    }
