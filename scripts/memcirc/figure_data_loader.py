...
Loads disk-verified JSONs into typed records (with ``is_placeholder`` flag)
...
Reproduce: ``env/bin/python3 scripts/memcirc/figure_data_loader.py``
(prints a smoke summary of every loader; does not write artefacts).

Sources.
* score_K AUROC and recon_cos   runs/memcirc/sae_dark_subspace/{dir}/results.json
* norm AUROC                    runs/memcirc/norm_baseline/{dir}/results.json (best_auroc)
* per-layer membership AUROC    runs/memcirc/behavioral_channels/{dir}/orthogonality.json
* bootstrap CIs (n_boot=10000)  runs/memcirc/sae_dark_subspace/{all_models_bootstrap_cis,new_bootstrap_cis}.json
* P69 N=6 noise-floor aggregate runs/memcirc/sae_noise_floor/p69_aggregate.json
* P12B fresh-init aggregate     runs/sae_array/p12b_freshinit/aggregate.json
                                (with single-shot layer18 fallback)

P12B placeholder. When the fresh-init aggregate is not present, callers should
use ``load_p12b_scaling_auroc()`` which returns a ``PlaceholderValue`` whose
``.value`` field contains the current best estimate from disk and whose
``.is_placeholder`` flag tells downstream plots to draw an open-face or lighter
marker.
"""
...
RUNS = REPO_ROOT / "runs"
DARK = RUNS / "memcirc" / "sae_dark_subspace"
NORM = RUNS / "memcirc" / "norm_baseline"
BC = RUNS / "memcirc" / "behavioral_channels"
NOISE_FLOOR = RUNS / "memcirc" / "sae_noise_floor"
SAE_ARRAY = RUNS / "sae_array"
...
MODEL_REGISTRY = {
    ...
    "Pythia-1B":    {"dark": "p1b_epoch5_v2",       "norm": "p1b_epoch5",   "bc": "p1b_epoch5",             "family": "GPT", "params": 1e9},
    "Pythia-2.8B":  {"dark": None,                  "norm": None,           "bc": "p2.8b_epoch5",           "family": "GPT", "params": 2.8e9},
    "Pythia-6.9B":  {"dark": "p69_epoch5_v2",       "norm": "p69_epoch5",   "bc": "p69_epoch5",             "family": "GPT", "params": 6.9e9},
    "Pythia-12B":   {"dark": "p12b_epoch5_v2",      "norm": "p12b_epoch5",  "bc": "p12b_epoch5",            "family": "GPT", "params": 12e9},
    # ── Other GPT-family
    "GPT-Neo-2.7B": {"dark": "neo_epoch5_v2",       "norm": "neo_epoch5",   "bc": "neo_epoch5",             "family": "GPT", "params": 2.7e9},
    "OPT-6.7B":     {"dark": "opt67_epoch5_v2",     "norm": "opt67_epoch5", "bc": "opt67_epoch5",           "family": "GPT", "params": 6.7e9},
    "Falcon-7B":    {"dark": "falcon_epoch5",       "norm": "falcon_epoch5","bc": "falcon7b_epoch5_v2",     "family": "GPT", "params": 7e9},
    # ── LLaMA / similar
    "Mistral-7B":   {"dark": "mistral_epoch5_v2",   "norm": "mistral_epoch5","bc": "mistral_epoch5_v2",     "family": "LLaMA","params": 7e9},
    "Llama-3-8B":   {"dark": "llama3_epoch5",       "norm": "llama3_epoch5", "bc": "llama3_epoch5_v2",      "family": "LLaMA","params": 8e9},
    "Qwen2-7B":     {"dark": "qwen2_epoch5_v2",     "norm": "qwen2_epoch5", "bc": "qwen2_epoch5",           "family": "LLaMA","params": 7e9},
...
class PlaceholderValue:
    """Wraps a numeric value that is provisional pending a canonical aggregate.

    Attributes
    ----------
    value      : current best on-disk estimate
    is_placeholder : True until the canonical aggregate lands
...
def load_p12b_scaling_auroc() -> PlaceholderValue:
    ...
    fallback. The ``is_placeholder`` flag is False for the aggregate path
    and True for the fallback.
    """
    ...
    if agg.exists():
        ...
        if mean is not None:
            return PlaceholderValue(value=float(mean), is_placeholder=False, source=str(agg))
    bl = load_best_layer_membership_auroc("Pythia-12B")
    return PlaceholderValue(
        value=bl["auroc"],
        is_placeholder=True,
        source=bl["source"] + " (canonical aggregate pending, placeholder)",
    )
...
def get_norm_table(model_labels: list[str]) -> dict:
    ...
    for lbl in model_labels:
        ...
        ds = load_dark_subspace(lbl) if MODEL_REGISTRY[lbl]["dark"] else None
        bcd = ds["original"]["score_K_auroc"] if ds else None
        out[lbl] = {
            "norm_auroc": n["auroc"],
            "bcd_auroc":  bcd,
            "source_norm": n["source"],
            "source_bcd":  ds["source"] if ds else None,
        }
    return out


def get_scaling_curve_data(model_labels: list[str]) -> dict:
    """Return {labels: [...], params: [...], aurocs: [...], placeholders: [...], sources: [...]}
    for the model-size scaling curve. `placeholders[i]` is True when value is provisional."""
    labels, params, aurocs, placeholders, sources = [], [], [], [], []
    for lbl in model_labels:
        if lbl == "Pythia-12B":
            ph = load_p12b_scaling_auroc()
            aurocs.append(ph.value)
            placeholders.append(ph.is_placeholder)
            sources.append(ph.source)
        else:
...
if __name__ == "__main__":
    ...
    for lbl, p, a, ph in zip(sc["labels"], sc["params"], sc["aurocs"], sc["placeholders"]):
        flag = "  (PLACEHOLDER)" if ph else ""
