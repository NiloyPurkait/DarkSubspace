...
Adds ``scripts/shared/`` and ``scripts/memcirc/`` to ``sys.path`` and routes
...
_MEMCIRC_DIR = Path(__file__).resolve().parent
_SHARED_DIR = _MEMCIRC_DIR.parent / "shared"
...
# memcirc scripts dir (for inter-script imports).
_memcirc_str = str(_MEMCIRC_DIR)
if _memcirc_str not in sys.path:
    sys.path.insert(0, _memcirc_str)
