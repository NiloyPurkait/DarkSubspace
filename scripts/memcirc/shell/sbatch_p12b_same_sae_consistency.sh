...
# Reproduce. sbatch scripts/memcirc/shell/sbatch_p12b_same_sae_consistency.sh
#
...
# highest recon_cos.
#
# Reuses scripts/memcirc/subspace_ablation_eval.py (the canonical error-PC
...
# highest recon_cos among landed seeds 47..51 if seed 47 fails the gate.
PICK_SEED=$(${PY} - <<'PYEOF'
import json, glob, os
candidates = []
# Canonical mixed-data SAE path is runs/memcirc/sae_dark_subspace/p12b_mixed_sae_seed{47..51}/.
for d in sorted(glob.glob("runs/memcirc/sae_dark_subspace/p12b_mixed_sae_seed*")):
...
OUT_DIR="runs/memcirc/causal_ablation/p12b_errPC_K10_schemaA_seed${PICK_SEED}"
mkdir -p "$OUT_DIR"

echo "=== K=10 PC ablation on Schema A mixed SAE (seed $PICK_SEED) ==="
${PY} scripts/memcirc/subspace_ablation_eval.py \
