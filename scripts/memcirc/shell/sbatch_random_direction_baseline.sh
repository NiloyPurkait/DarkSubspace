...
# Reproduce: sbatch scripts/memcirc/shell/sbatch_random_direction_baseline.sh

set -eu
cd "$(dirname "$0")/../../.."
mkdir -p logs runs/memcirc/random_direction_baseline

PY="env/bin/python3"
SCRIPT="scripts/memcirc/random_direction_baseline.py"
...
${PY} ${SCRIPT} \
  ...
  --layer 16 \
  --output-path runs/memcirc/random_direction_baseline/p69_layer16.json \
...
${PY} ${SCRIPT} \
  ...
  --member-texts ${MEM} --nonmember-texts ${NON} \
  --layer 16 \
  --output-path runs/memcirc/random_direction_baseline/neo_layer16.json \
  --n-directions 100 --seed 42
...
${PY} ${SCRIPT} \
  ...
  --member-texts ${MEM} --nonmember-texts ${NON} \
  --layer 16 \
  --output-path runs/memcirc/random_direction_baseline/qwen2_layer16.json \
  --n-directions 100 --seed 42
...
${PY} ${SCRIPT} \
  ...
  --member-texts ${MEM} --nonmember-texts ${NON} \
  --layer 18 \
  --output-path runs/memcirc/random_direction_baseline/p12b_layer18.json \
  --n-directions 100 --seed 42

echo ""
echo "=== Random-direction baseline done: $(date) ==="
echo "Outputs: runs/memcirc/random_direction_baseline/{p69,neo,qwen2,p12b}_layer*.json"
