...
Reproduce: ``env/bin/python3 scripts/memcirc/plot_paper5_figures.py``
...
The Pythia-12B scaling-curve AUROC may be provisional. The loader function
``load_p12b_scaling_auroc()`` returns a placeholder when the canonical
``runs/sae_array/p12b_freshinit/aggregate.json`` is not present, and the marker
is drawn with an open face and an annotation in that case.
...
OUT = pathlib.Path(os.environ.get("FIGDIR", REPO_ROOT / "paper5" / "figures"))
...
def fig_scaling_curve():
    ...
    # Markers.open face for placeholder
    ...
    is_ph = placeholders[idx_12b]
    note  = "(canonical aggregate pending)" if is_ph else "(layer-selection\n artifact)"
    ax.annotate(note,
