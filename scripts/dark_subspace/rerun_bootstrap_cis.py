...
Targets.
    standard_probes  reruns AUROC, TPR@1%FPR, TPR@5%FPR bootstrap CIs for
                     ...
                     runs/dark_subspace/baseline_attacks/<model>/results.json in
    ...
    paraphrase       reruns the delta_paraphrased_minus_original AUROC and
                     ...
                     runs/dark_subspace/paraphrase_sensitivity/<m>/results.json
                     in place.
    bow              reruns the BoW-ceiling bootstrap (per variant) at
                     ...
                     30s. Updates
                     runs/dark_subspace/bow_ceiling/memcirc_ctrl_ft/results.json
                     in place.
...
PARAPHRASE_ROOT = REPO_ROOT / "runs" / "dark_subspace" / "paraphrase_sensitivity"
BOW_ROOT = REPO_ROOT / "runs" / "dark_subspace" / "bow_ceiling" / "memcirc_ctrl_ft"
...
def rerun_standard_probes(model: str) -> dict:
    ...
    results.setdefault("bootstrap_history", []).append({
        "n_boot": N_BOOT_NEW,
        "seed": SEED_NEW,
        "rerun_script": "scripts/dark_subspace/rerun_bootstrap_cis.py",
