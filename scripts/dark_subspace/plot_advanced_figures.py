...
Reproduce: ``.venv/bin/python scripts/dark_subspace/plot_advanced_figures.py``
...
The scaling-curve panel draws an open-face marker and skips its CI band in
that case.
...
def fig3_scaling_curve():
    ...
    provisional = np.array(sc["provisional"], dtype=bool)
    ...
    for i, k in enumerate(keys):
        if provisional[i]:
            continue  # skip CI band when the value uses a single-run source
    ...
    note = "single-run\nsource" if provisional[idx] else "capacity\nsaturation?"
    ax.annotate(note,
...
if __name__ == "__main__":
    print("=" * 60)
    print("Generating Dark Subspace advanced figures")
    print("=" * 60)
